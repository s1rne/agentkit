...
const en = {
  ...
  // providers / usage / boxes
  providersHeader: "providers",
  providersReady: "ready",
  providersNotLoggedIn: "not logged in",
  providersAbsent: "not installed",
  providersMetered: "METERED AUTH — will be billed per token, never selected",
  providersCoreNote: "The core works on claude-code alone. Other providers only add capability.",
  providersWritten: "provider state: .agentkit/state/PROVIDERS.md",
  riskyEnv: (list) => `metered API credentials in the environment: ${list}. Unset them — agents run on subscription CLIs only`,
  contextHeader: "context",
  contextLine: (t, w, pct) => `session context: ${t} / ${w} (${pct}%)`,
  contextRotate: (pct) => `above ${pct}% — wrap the session and start a fresh one before quality drops`,
  contextNoData: "no local transcript for this project yet",
  usageHeader: "usage",
  usageWindow: "rolling 5-hour window",
  usageToday: "since midnight",
  usageRow: (label, v) => `${label.padEnd(16)} ${v}`,
  boxesHeader: "boxes",
  boxesEmpty: "no boxes",
  boxKept: (id, why) => `kept ${id}: ${why}`,
  boxRemoved: (id) => `removed ${id}`,
  capacityLine: (c) =>
    `${c.perfCores} of ${c.cores} cores · ${c.ramAvailGB} of ${c.ramTotalGB} GB RAM free${c.dedicated ? " (container limit)" : ""} · ${c.diskFreeGB} GB disk free · load ${c.load1}`,
  concurrency: (n) => `parallel agents allowed right now: ${n}`,
  admitsNo: (why) => `no room for another agent: ${why}`,
  ...
  ${b("npx @s1rne/agentkit adopt")} [--lang en|ru]   take over an existing .claude/ instead of overwriting it
  ...
  ${b("npx @s1rne/agentkit wave")} [--conc n] [--max n] [--budget n]   run the whole queue by itself
  ${b("npx @s1rne/agentkit run")} <task> [--role r] [--wait] [--priority] [--mode m] [--dry-run]
  ${b("npx @s1rne/agentkit team")} [--watch] [<task|run>]   who is working, on what, at what cost
  ${b("npx @s1rne/agentkit providers")}  which CLIs are available and what they add
  ${b("npx @s1rne/agentkit account")} add <provider> <id> · list · remove <id>
  ${b("npx @s1rne/agentkit context")}    how full this session's context is
  ${b("npx @s1rne/agentkit usage")}      tokens spent in the current window
  ${b("npx @s1rne/agentkit box")} list|gc [--force]

  ${b("--json")} on status, providers, team, box, context and usage prints the same data unformatted, for a program.
...
const ru = {
  ...
  providersReady: "готов",
  providersNotLoggedIn: "не авторизован",
  providersAbsent: "не установлен",
  providersMetered: "ПЛАТНЫЙ ДОСТУП — тарифицируется по токенам, никогда не выбирается",
  providersCoreNote: "Ядро работает на одном claude-code. Остальные провайдеры только добавляют возможности.",
  providersWritten: "состояние провайдеров: .agentkit/state/PROVIDERS.md",
  riskyEnv: (list) => `в окружении ключи платного API: ${list}. Убери их — агенты работают только через подписочные CLI`,
  contextHeader: "контекст",
  contextLine: (t, w, pct) => `контекст сессии: ${t} / ${w} (${pct}%)`,
  contextRotate: (pct) => `больше ${pct}% — заверши сессию и начни новую, пока не поехало качество`,
  contextNoData: "локальных транскриптов по этому проекту пока нет",
  usageHeader: "расход",
  usageWindow: "скользящее окно 5 часов",
  usageToday: "с полуночи",
  usageRow: (label, v) => `${label.padEnd(16)} ${v}`,
  boxesHeader: "боксы",
  boxesEmpty: "боксов нет",
  boxKept: (id, why) => `оставлен ${id}: ${why}`,
  boxRemoved: (id) => `удалён ${id}`,
  capacityLine: (c) =>
    `${c.perfCores} из ${c.cores} ядер · ${c.ramAvailGB} из ${c.ramTotalGB} ГБ RAM свободно${c.dedicated ? " (предел контейнера)" : ""} · ${c.diskFreeGB} ГБ на диске · нагрузка ${c.load1}`,
  concurrency: (n) => `параллельных агентов сейчас можно: ${n}`,
  admitsNo: (why) => `места для ещё одного агента нет: ${why}`,
  ...
  ${b("npx @s1rne/agentkit init")} [--lang en|ru] [--pack base|web-product|full] [--adapters claude-code,cursor,agents-md] [--force]
  ${b("npx @s1rne/agentkit adopt")} [--lang en|ru]   принять существующий .claude/, а не затереть его
  ...
  ${b("npx @s1rne/agentkit wave")} [--conc n] [--max n] [--budget n]   ведёт всю очередь сама
  ${b("npx @s1rne/agentkit run")} <задача> [--role r] [--wait] [--priority] [--mode m] [--dry-run]
  ${b("npx @s1rne/agentkit team")} [--watch] [<задача|запуск>]   кто работает, над чем и почём
  ${b("npx @s1rne/agentkit providers")}  какие CLI доступны и что они добавляют
  ${b("npx @s1rne/agentkit account")} add <провайдер> <id> · list · remove <id>
  ${b("npx @s1rne/agentkit context")}    насколько заполнен контекст этой сессии
  ${b("npx @s1rne/agentkit usage")}      расход токенов за текущее окно
  ${b("npx @s1rne/agentkit box")} list|gc [--force]

  ${b("--json")} у status, providers, team, box, context и usage печатает те же данные без оформления, для программы.
