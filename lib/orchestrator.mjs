...
/**
 * «Не может менять» и «не может выполнять» — разные вещи.
 *
 * Ревьюирующая роль получает бокс readonly, а тот раньше означал plan-режим,
 * который запрещает и запуск. На живом прогоне критик разобрал реализацию по
 * каждому приёмочному критерию, дефектов не нашёл — и всё равно вернул задачу,
 * потому что обязательный прогон выполнить не мог. Свой же definition-of-done
 * требует проверки запуском, а роль, которая обязана её подтвердить, была
 * единственной, кому запуск закрыт: ревью вырождалось в чтение, а «доработать»
 * означало «я не смог посмотреть» — и стоило лишнего прогона исполнителя.
 *
 * Запись закрывается списком запрещённых инструментов, а не режимом, поэтому
 * между `read` и `edit` есть `verify`: запускать можно, писать нельзя.
 */
const strictness = { read: 0, verify: 1, edit: 2, isolated: 3 };
...
/** Tools a role declares for itself; an empty list means it declares no limits. */
function declaredTools(root, role) {
...
export function roleWrites(root, role) {
  const tools = declaredTools(root, role);
  if (!tools.length) return true;
  return tools.some((t) => /^(Write|Edit|NotebookEdit)$/i.test(t));
}

/**
 * Может ли роль выполнять команды — по тому же признаку и из того же источника:
 * критик и аудитор объявляют `Bash`, аналитику предметной области он не нужен.
 * Роль, которой запуск не объявлен, и не получит его.
 */
export function roleRuns(root, role) {
  const tools = declaredTools(root, role);
  if (!tools.length) return true;
  return tools.some((t) => /^Bash$/i.test(t));
}
...
export async function run(root, cfg, opts) {
  ...
  /**
   * Риск и область берутся из файла задачи, если вызывающий их не задал.
   *
   * Раньше учитывалось только переданное: задача с `risk: high` в своём файле
   * получала общий бокс и писала прямо в рабочий каталог человека, потому что
   * вызывающий забыл продублировать флаг. Объявление задачи и есть источник
   * правды — вызывающий может его ужесточить, но не должен обязан повторять.
   */
  const declared = task ? findTask(root, task) : null;
  const meta = declared && fs.existsSync(declared) ? parseFront(fs.readFileSync(declared, "utf8")).data : {};
  const risk = opts.risk && opts.risk !== "normal" ? opts.risk : meta.risk || "normal";
  const kind = opts.kind && opts.kind !== "normal" ? opts.kind : meta.kind || "normal";
  const role = opts.role || meta.owner || "backend-dev";

  /**
   * Пределы берутся из настроек проекта, если вызывающий не задал свои.
   *
   * Раньше без явной передачи молча действовали дефолты: волна не передавала
   * их, и агенты убивались по 20-минутному потолку, притом что в проекте стояло
   * 45. Настройка, которую можно не заметить, — это не настройка.
   */
  const limits = { ...DEFAULT_LIMITS, ...(cfg.providers?.limits || {}), ...(optLimits || {}) };
  ...
  let gate = admits(activeRuns(root).length, limits, undefined, { priority });
  if (!gate.ok && wait) {
    // Ждать место — работа запускателя. Иначе каждый вызывающий изобретает свой
    // цикл повторов, и один из них обязательно сочтёт «нет места» провалом.
    const until = Date.now() + (typeof wait === "number" ? wait : 30 * 60_000);
    while (!gate.ok && Date.now() < until) {
      await new Promise((r) => setTimeout(r, Math.min(gate.waitMs || 15000, 20000)));
      gate = admits(activeRuns(root).length, limits, undefined, { priority });
    }
  }
  ...
  /**
   * Право «запускать, но не писать» держится на том, что инструменты записи
   * отзываются поимённо. Провайдер, который так не умеет, оставляет читающую
   * роль читающей — иначе `verify` тихо превратился бы в полный доступ.
   */
  const mayVerify = provider?.canRunWithoutWriting !== false && roleRuns(root, role);
  const level = (boxMode) => {
    const base = PERMISSION_BY_BOX[boxMode] || "edit";
    return base === "read" && mayVerify ? "verify" : base;
  };
  const wanted = level(decided.mode);
  const given = level(box.mode);
  ...
  if (dryRun) {
    ...
    return { ...report, status: "dry-run", command: shown, promptChars: prompt.length, limits };
  }
