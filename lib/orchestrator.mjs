...
Keep it under 25 lines and use exactly these sections:
...
export function composePrompt(root, { role, taskFile, instruction }) {
  const parts = [];
...
export async function run(root, cfg, opts) {
  ...
  const prompt = composePrompt(root, { role, taskFile: task ? findTask(root, task) : null, instruction });
  ...
  trackActive(root, { runId, pid: child.pid, task, role, provider: routed.provider, startedAt: started, parent, depth });
