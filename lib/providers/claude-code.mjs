...
/**
 * The headless way into the same subscription: `claude setup-token` issues a
 * year-long OAuth token, which is the only way to bring an agent up in a
 * container without a browser. It requires Pro/Max/Team/Enterprise and can do
 * nothing but call the model, so it is never the metered path.
 */
export const tokenEnv = "CLAUDE_CODE_OAUTH_TOKEN";

/**
 * Two ways into a subscription, verified against the live CLI:
 *
 *   browser login   {"loggedIn":true,"authMethod":"claude.ai","apiProvider":"firstParty",
 *                    "email":"…","subscriptionType":"max"}
 *   CLAUDE_CODE_OAUTH_TOKEN
 *                   {"loggedIn":true,"authMethod":"oauth_token","apiProvider":"firstParty"}
 *
 * Reading only the first pair as a subscription marked a perfectly legitimate
 * login "metered", and a metered login is never selected — so a container that
 * had no other way to authenticate could not run an agent at all.
 */
const SUBSCRIPTION_AUTH = ["claude.ai", "oauth_token"];
...
export async function probe({ configDir } = {}) {
  ...
  const viaEnvToken = typeof env[tokenEnv] === "string" && env[tokenEnv].trim() !== "";
  ...
  return readAuth(data, { viaEnvToken });
}

/**
 * What `claude auth status` says about this login, as a probe result.
 *
 * Note what the CLI does NOT do: a token in the environment is reported as
 * logged in without being checked against the server, so an expired or
 * mistyped one still reads `loggedIn:true` here. The difference shows up on the
 * first real run, where the account bookkeeping takes a failing login out of
 * rotation. A probe that never calls the model cannot know more than this.
 */
export function readAuth(data, { viaEnvToken = false } = {}) {
  ...
  const detail = {
    ...
    // How this login was made — a token login carries no address, so this is
    // the only thing that tells the human which of the two they are looking at.
    loginPath: authMethod === "oauth_token" ? "token" : authMethod === "claude.ai" ? "browser" : null,
    // The token comes from the environment, which every config directory shares:
    // separate logins cannot be built on it.
    viaEnvToken: authMethod === "oauth_token" && viaEnvToken,
  };

  if (!data.loggedIn) return state("not-logged-in", detail, LOGIN_HINT);

  const subscription = SUBSCRIPTION_AUTH.includes(detail.authMethod) && detail.apiProvider === "firstParty";
  if (!subscription) {
    return state(
      "metered",
      detail,
