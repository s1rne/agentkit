...
/** How a box mode maps onto this CLI's permission modes. */
const PERMISSION = {
  read: "plan",              // reviewing roles: analysis only, no edits
...
export function spawnSpec({ prompt, model, cwd, configDir, permission = "edit", extraArgs = [] } = {}) {
  ...
  if (permission === "read") args.push("--disallowed-tools", "Edit", "Write", "NotebookEdit");
...
export function parseStream(text) {
  ...
  const report = failed && streamed.length > final.length ? streamed : final || streamed;
    ...
    error: failed
      ? String(
          result.api_error_status ||
            (typeof result.result === "string" && /error|closed|timeout|limit/i.test(result.result)
              ? result.result.split("\n")[0].slice(0, 160)
              : "") ||
