...
/**
 * false: there is no way to withhold the write tools by name here either, so
 * "run but do not write" cannot be expressed — a role that needs it stays in
 * plan mode, where it can neither write nor run.
 */
export const canRunWithoutWriting = false;
...
export function spawnSpec({ prompt, model, cwd, configDir, home, permission = "edit", extraArgs = [] } = {}) {
  ...
  // `plan` is this CLI's read-only mode; --force would defeat it. `verify`
  // cannot be honoured here — running without writing is not expressible — so
  // it lands in plan too rather than quietly turning into full write access.
  if (permission === "read" || permission === "verify") args.push("--mode", "plan");
