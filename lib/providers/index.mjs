...
export async function probeAll({ accounts = [], ttlMs = 300000, cacheFile } = {}) {
   ...
   *
   * Usually the identity is the address. A login made from a token in the
   * environment has no address and belongs to no config directory either — it
   * is one token every account is reading, so those entries are the same login
   * just as surely.
   */
  const identity = (a) =>
    a.detail?.email || (a.detail?.viaEnvToken ? `${a.provider} token in the environment` : null);
  const byIdentity = new Map();
  for (const a of accountResults) {
    const who = identity(a);
    if (!who) continue;
    const key = `${a.provider}:${who}`;
    if (byIdentity.has(key)) {
      const first = byIdentity.get(key);
      a.duplicateOf = first.id;
      a.hint = `same login as "${first.id}" (${who}) — isolation did not take effect`;
...
export function summary(probeResult) {
  ...
  const rows = PROVIDERS.map((p) => {
    ...
    // A token login reports no plan and no address, so name the path instead:
    // "ready" with nothing beside it would look like a login nobody can place.
    const who = e.detail?.subscriptionType
      ? ` (${e.detail.subscriptionType})`
      : e.detail?.loginPath === "token"
        ? ` (${claudeCode.tokenEnv})`
        : "";
