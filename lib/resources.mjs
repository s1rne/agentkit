...
export const DEFAULT_LIMITS = {
  agentRssMB: 3000, // hard per-agent ceiling before the watchdog kills it
  estAgentRssMB: 1200, // what we assume one agent needs when deciding to admit
 ...
 */
export function admits(running = 0, limits = DEFAULT_LIMITS, cap = capacity()) {
  const l = { ...DEFAULT_LIMITS, ...limits };
  const max = maxConcurrency(l, cap);
...
export function watch(pid, opts = {}) {
  ...
  timer = setInterval(() => {
    ...
    if (rss > maxRssMB) return terminate(`Agent ${pid} used ${rss} MB RSS, ceiling is ${maxRssMB} MB`);
