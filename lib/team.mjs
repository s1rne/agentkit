...
export function gather(root, cfg = {}) {
  ...
  // Реестр запусков общий на машину: две сборки на одном ноутбуке делят память,
  // и потолок мест обязан быть общим. Но в списке «в работе» чужие задачи
  // выглядят как свои, поэтому показываем только запуски этого проекта.
  const all_ = activeRuns(root);
  const mine = all_.filter((r) => !r.root || path.resolve(r.root) === path.resolve(root));
  const active = mine.map((r) => {
  ...
  return {
    active,
    elsewhere: all_.length - mine.length,
    capacity: cap,
...
/**
 * Пересекаются ли области двух задач.
 *
 * Сравнение `touches` как строк недостаточно: `packages/db` и `packages/db/client`
 * — разные строки и один пакет. На живом прогоне это стоило двух несовместимых
 * контрактов, разобранных архитектором. Путь сравнивается как путь: по префиксу
 * с границей сегмента.
 */
export function overlap(a, b) {
  for (const x of touchesOf(a)) {
    for (const y of touchesOf(b)) {
      if (x === y || x.startsWith(`${y}/`) || y.startsWith(`${x}/`)) return { a: x, b: y };
    }
  }
  return null;
}

/** Пары задач, которые нельзя запускать одновременно. */
export function collisions(tasks) {
  const list = [...tasks].filter(Boolean);
  const out = [];
  for (let i = 0; i < list.length; i++) {
    for (let j = i + 1; j < list.length; j++) {
      const o = overlap(list[i], list[j]);
      if (o) out.push({ a: list[i].id, b: list[j].id, at: o });
    }
  }
  return out;
}
