...
export function parseFront(text) {
  ...
  for (const line of m[1].split(/\r?\n/)) {
    ...
    // An empty field is an absent value, not the four-letter string "null".
    // A planner writes `blocked_by: null` for a task that depends on nothing,
    // and every reader that treats the field as a list then sees one dependency
    // on a task called "null" — which never exists, so the task never runs.
