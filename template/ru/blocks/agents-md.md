...
### Где агенты работают и на чём

У каждой задачи есть бокс — `readonly`, `shared`, `worktree` или `sandbox` — его выбирает лид и
записывает в файл задачи; см. `.agentkit/skills/workspace-protocol.md`. Слияние — отдельная
задача, и никогда до критика. Агенты работают только через подписочные CLI, никогда через
платный API-ключ; роль просит возможность, а не провайдера. Всё работает на одном Claude Code.
См. `provider-routing`, `resource-limits`, `context-budget`.
